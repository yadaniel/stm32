...
#include "../Drivers/STM32F4xx_HAL_Driver/Inc/stm32f4xx_hal_i2c_ex.h"
...
int8_t user_i2c_read(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, dev_id << 1, &reg_addr, 1, 100) != HAL_OK) {
        return BME680_E_NOTOK;
    }
    if(HAL_I2C_Master_Receive(&hi2c1, dev_id << 1, reg_data, len, 100) != HAL_OK) {
        return BME680_E_NOTOK;
    }
...
int8_t user_i2c_write(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, dev_id << 1, &buffer[0], len+1, 100) != HAL_OK) {
        return BME680_E_NOTOK;
    }
