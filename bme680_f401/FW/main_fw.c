...
void UART_Sendchar(uint8_t c) {
    while(!(USART2->SR & USART_SR_TXE)); // block until tx empty
    USART2->DR = c;
}

uint8_t UART_Getchar() {
    uint8_t c;
    while(!(USART2->SR & USART_SR_RXNE));  // nothing received so just block
    c = USART2->DR; // read Receiver buffer register
    return c;
}
...
int main_fw(void) {
    ...
    //gas_sensor.dev_id = 0x77;   // SDO pulled high
    ...
    while(1) {
        ...
        if(rslt == BME680_OK) {
            ...
            //bme_exit(rslt);   // no new data error
        }
